...
class lessc_formatter_classic {
	...
	public function block($block) {
		...
		if (!empty($block->selectors)) {
			...
			if ($isSingle) {
				echo esc_html($this->openSingle);
				$inner = "";
			} else {
				echo esc_html($this->open . $this->break);
		...
		if (!empty($block->lines)) {
			...
			echo esc_html($inner) . implode($glue, $block->lines);
			if (!$isSingle && !empty($block->children)) {
				echo esc_html($this->break);
			}
		}
		...
		if (!empty($block->selectors)) {
			if (!$isSingle && empty($block->children)) echo esc_html($this->break);

			if ($isSingle) {
				echo esc_html($this->closeSingle . $this->break);
			} else {
				echo esc_html($pre . $this->close . $this->break);
			}
