<?php
    ...
    $tip_title = __( 'Developer Mode Enabled', 'sage' );
    ...
    if ( $this->parent->dev_mode_forced ) {
        ...
        if ( Redux_Helpers::isWpDebug() ) {
            $is_debug  = true;
            $debug_bit = __( 'WP_DEBUG is enabled', 'sage' );
        }

        $localhost_bit = '';
        if ( Redux_Helpers::isLocalHost() ) {
            $is_localhost  = true;
            $localhost_bit = __( 'you are working in a localhost environment', 'sage' );
        }

        $conjunction_bit = '';
        if ( $is_localhost && $is_debug ) {
            $conjunction_bit = ' ' . __( 'and', 'sage' ) . ' ';
        }

        $tip_msg = __( 'This has been automatically enabled because', 'sage' ) . ' ' . $debug_bit . $conjunction_bit . $localhost_bit . '.';
    } else {
        $tip_msg = __( 'If you are not a developer, your theme/plugin author shipped with developer mode enabled. Contact them directly to fix it.', 'sage' );
    }
        ...
        <div class="display_header">
            ...
            <?php if ( isset( $this->parent->args['dev_mode'] ) && $this->parent->args['dev_mode'] ) { ?>
                <div class="redux-dev-mode-notice-container redux-dev-qtip"
                    ...
                    <span
                        class="redux-dev-mode-notice"><?php _e( 'Developer Mode Enabled', 'sage' ); ?></span>
                </div>
            <?php } elseif (isset($this->parent->args['forced_dev_mode_off']) && $this->parent->args['forced_dev_mode_off'] == true ) { ?>
                ...
                <div class="redux-dev-mode-notice-container redux-dev-qtip" 
                     ...
                     qtip-content="<?php echo esc_attr( $tip_msg ); ?>">
                    <span
                        class="redux-dev-mode-notice" style="background-color: #FF001D;"><?php _e( 'FORCED DEV MODE OFF ENABLED', 'sage' ); ?></span>
                </div>
